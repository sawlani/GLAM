...
class ICAGraphGenerator(GraphGenerator):
	def __init__(self, classRatio, h, **kwargs):
		super().__init__(len(classRatio))
		self.classRatio = classRatio
		self.h = h
	
	def connect_node(self, G, source_node=None):
		if source_node is None:
			if len(G.nodes) == 0:
				return
			source_node = random_state.choice(list(G.nodes))
		neighbor = self.select_neighbor(G, source_node)
		if neighbor is not None:
			G.add_edge(source_node, neighbor)
			return neighbor
		else:
			return None

	def select_neighbor(self, G:nx.Graph, source_node):
		pr = dict()
		source_label = G.nodes[source_node]["color"]
		for v in G.nodes:
			if v == source_node:
				pr[v] = 0
			else:
				degree_v = float(max(G.degree[v], 1))
				if G.nodes[v]['color'] == source_label:
					pr[v] = float(degree_v) * self.h
				else:
					pr[v] = float(degree_v) * (1 - self.h) / (self.numClass - 1)
		...
		if norm_pr == 0:
			...
			for v in list(pr.keys()):
				pr[v] = float(pr[v]) / norm_pr
			neighbor = random_state.choice(list(pr.keys()), None, False, list(pr.values()))
			return neighbor

	def add_node(self, G, nodeID):
		if self.__coloriter:
			label = next(self.__coloriter)
		else:
			label = random_state.choice(list(range(1, len(self.classRatio) + 1)), 1, False, self.classRatio)[0]
		G.add_node(nodeID, color=label)
		self.connect_node(G, nodeID)

	def generate_graph(self, n, alpha=0.5):
		'''
		n: Number of nodes in the synthetic graph
		alpha: control node to edge ratio. Expected number of edges are 1 / (1-alpha) * n
		...
		if n > 1 and np.sum(self.classRatio) == n:
			...
			self.__colorlist = []
			for classID, classSize in enumerate(self.classRatio):
				self.__colorlist += [classID + 1] * int(classSize)
		else:
			class_size_e = np.array(self.classRatio) * n
			class_size_i = np.floor(class_size_e)
			class_size_r = class_size_e - class_size_i
			numNode_i = int(np.sum(class_size_i)); numNode_r = int(n - numNode_i)
			self.__colorlist = []
			for classID, classSize in enumerate(class_size_i):
				self.__colorlist += [classID + 1] * int(classSize)
			self.__colorlist += list(np.argsort(class_size_r)[::-1][:numNode_r] + 1)
		random_state.shuffle(self.__colorlist)
		self.__coloriter = iter(self.__colorlist)
	...
	def __call__(self, n, alpha=0.5):
		return self.generate_graph(n, alpha)

if __name__ == "__main__":
	generator = MixhopGraphGenerator([20, 20, 20], heteroWeightsExponent=2.0)
	G = generator(60, 2, 20, 0)
	colors = []
	for v in G.nodes():
		colors.append(G.nodes[v]['color'])
	nx.draw(G, pos=nx.circular_layout(G), node_color=colors, node_size=25)
	plt.draw()
	plt.show()
