...
def train(args, model, device, train_graphs, optimizer, epoch):
    ...
    #total_iters = (len(train_graphs)//args.batch_size) + 1
    #pbar = tqdm(range(0, len(train_graphs), args.batch_size), unit='batch')

    #loss_accum = 0
    perm = torch.randperm(len(train_graphs))

    #embeddings = [None]*len(train_graphs)

    #for i in range(0, len(train_graphs), args.batch_size):
        #selected_idx = perm[i:i + args.batch_size]

        #batch_graph = [train_graphs[idx] for idx in selected_idx]
    batch_graph = train_graphs
    output = model(batch_graph, -1)
        #batch_embeddings = model.forward_new(batch_graph, -1)

    #    for idx, emb in zip(selected_idx, batch_embeddings):
    #        embeddings[idx] = emb

    #MMD_values = compute_mmd_gram_matrix(embeddings)

    #output = model2(MMD_values)
...
def main():
    ...
    if args.dataset == "mixhop":
        graphs, num_classes = load_synthetic_data(10)
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #acc_test = test(args, model, model2, device, test_graphs)
