...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, input_dim, hidden_dim, output_dim, final_dropout, learn_eps, graph_pooling_type, neighbor_pooling_type, device):
        '''
        ...
        for layer in range(self.num_layers-1):
            if layer == 0:
                self.mlps.append(MLP2layer(input_dim, hidden_dim, hidden_dim))
            else:
                self.mlps.append(MLP2layer(hidden_dim, hidden_dim, hidden_dim))
        ...
        #self.svm = nn.Linear(no_of_graphs,1)
    ...
    '''
    def forward2(self, batch_graph, output_layer):
        X_concat = torch.cat([graph.node_features for graph in batch_graph], 0).to(self.device)
