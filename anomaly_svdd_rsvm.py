...
def train_bigstep(args, model, device, train_graphs, optimizer, epoch, Z):
    model.train()
    ...
    for pos in pbar:        
        ...
        K_RZ = compute_mmd_gram_matrix(R_embeddings, Z_embeddings, gamma=gamma, type="MMD")
        
        #dists = torch.mean(K_RZ, dim=1)
        dists = torch.median(K_RZ, dim=1).values
...
def test(args, model, device, test_graphs, Z):
    model.eval()
    ...
    for start in range(0, len(test_graphs), args.batch_size):
        ...
        #dists_batch = torch.mean(K_RZ, dim=1)
        dists_batch = torch.median(K_RZ, dim=1).values
...
def main():
    ...
    for _ in radii:
        ...
        #PRE-TRAINING TEST
        score, dists = test(args, model, device, graphs, Z)
        ...
        for epoch in range(1, no_epochs + 1):
            ...
            avg_loss = train(args, model, device, graphs, optimizer, epoch, Z)
        ...
        fig.suptitle("SVDD with RSVM for fixed radius=%f" % args.radius)
        ...
        fig.savefig("radius"+str(args.radius)+".png", dpi=1500)
    ...
    axs[2,0].set(xlabel='Radii', ylabel='Non-reg Loss')
    axs[2,0].plot(radii, NON_REG_LOSSES)
    axs[2,0].grid()
    ...
    fig.savefig("SVDD_loss_vs_radius.png", dpi=1500)
    plt.show()
