...
class NTN(nn.Module):
    def __init__(self, input_dim):
        '''
            input_dim_1: dimensionality of input features
            input_dim_2: dimensionality of hidden units at ALL layers
            output_dim: number of classes for prediction
        '''
    
        super(NTN, self).__init__()

        self.linear = nn.Linear(input_dim**2 + 2*input_dim, 1)
    
    def forward(self, v1, v2):
        outer_product = torch.ger(v1, v2)
        x = torch.cat([outer_product.flatten(), v1, v2])
        x = self.linear(x)
        x = torch.sigmoid(x)
